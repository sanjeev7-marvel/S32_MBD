/*
 * sim_sil_target_AMMCLIB_s32k1xx_ssf.c
 *
 * Automatically generated s-function with I/O interface for:
 * Component: sim_sil_target_AMMCLIB_s32k1xx
 * Component Simulink Path: sim_sil_target_AMMCLIB_s32k1xx
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                sim_sil_target_AMMCLIB_s32k1xx_ssf
...
static RegMdlInfo rtMdlInfo_sim_sil_target_AMMCLIB_s32k1xx[1] = { "",
...

  {
...

    {
      mxArray * codeGenComponent = mxCreateString(
        "sim_sil_target_AMMCLIB_s32k1xx");
      mxArray *rhs[3];
      char * simulinkBlockPath = getSimulinkBlockPath(S);
      if (simulinkBlockPath == NULL) {
        ssSetErrorStatus(S,
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  if (S->mdlInfo->genericFcn != (NULL) &&
    ...
    ssRegModelRefMdlInfo(S, "sim_sil_target_AMMCLIB_s32k1xx",
                         rtMdlInfo_sim_sil_target_AMMCLIB_s32k1xx, 0);
  }
  ...
  {
    {
      ...
      /* dispatch command to the target */
      if (commandDispatch(S, IOBufferPtr, 11)!=XILHOSTAPPSVC_SUCCESS) {
        return;
      }                                /* if */
...

                      {
...

                        {
                          copyIOData(simDataMemUnitPtr, mxMemUnitPtr,
                                     &mxMemUnitPtr, num_elements, sizeof(int16_T));
                        }
                      }
                    }
                  }
